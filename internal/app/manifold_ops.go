...
func (a *App) initDomainModules(manifoldID model.ManifoldID) {
	...
	a.pumpRamp = NewPumpRamp(a.clk, a.cfg.PumpRampSteps, a.cfg.ProcessTick())
	a.pumpSched = clock.NewPumpScheduler(a.clk)
	a.inertWindow = clock.NewInertWindow(a.clk, time.Duration(a.cfg.InertWindowMin)*time.Minute)
	a.manifoldLeases = interlock.NewManifoldLeaseRegistry(30 * time.Second)
	a.leakGuard = interlock.NewManifoldLeakGuard(8.0)
	a.pumpCancels = make(map[model.PumpColumnID]context.CancelFunc)
	a.pumpFSM = fsm.NewPumpFSM(model.PumpColumnID("col-primary"), a.onPumpTransition)
	fsm.RegisterPumpBoostHook(a.pumpFSM.Hooks())
	a.alarms.Manager().Register("MANIFOLD_LEAK", "Manifold leak detected")
}
